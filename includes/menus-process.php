<?php 
	 ...
	 */
	if(isset($_POST['tntEditChannelCat']))
	{	
		$catID = $_POST["catID"];
		$c = new TNT_ChannelCat();
		$c->tntGetCat($catID);
		...
		if($c->tntUpdateChannelCat() >= 0)
		{
			$location = add_query_arg(array('m'=>1));
		}
		else if($t->tntUpdateChannelCat() == false)
		{
			$location = add_query_arg(array('m'=>0));
		}
		Header("Location: $location");
	 ...
	 */
	if(isset($_POST['tntDelChannelCat_Yes']))
	{
	 ...
	 */
	if(isset($_POST['tntDelChannelCat_No']))
	{
		$location = admin_url()."/admin.php?page=tnt_channel_cat_manager_page";
		Header("Location: $location");
	}
